...
// Each entry: [stop1_color, stop2_color, stop3_color_or_undefined].
// These replace the CSS linear-gradient() strings which are NOT valid SVG fill values
// and cause the scene rect to paint solid black.
const STYLE_GRADIENT_STOPS: Record<string, [string, string, string?]> = {
  cinematic_realism:       ['#2A1F15', '#5C3E28', '#3A2818'],
  watercolor:              ['#F1E8DE', '#D4B8A0', '#BFA08A'],
  storybook_illustration:  ['#C8DFC8', '#A8C4A8', '#7BA87B'],
  animated_3d:             ['#E0D0C0', '#C8A890', '#9A7858'],
  claymation:              ['#D4B090', '#B08060', '#8C5C38'],
  pencil_sketch:           ['#EDE8E0', '#D4CCBE', '#B0A898'],
  pixel_art:               ['#F0D080', '#C8A040', '#804010'],
  voxel_minecraft:         ['#70A030', '#508020', '#306010'],
};
...
const CardArt = ({ containerId, text, aspectRatio, artStyle, cardType = 'caption' }: CardArtProps) => {
  ...
  // Use stop colors for SVG <linearGradient> — CSS linear-gradient() is not a valid SVG fill.
  const gradientStops = STYLE_GRADIENT_STOPS[artStyle as keyof typeof STYLE_GRADIENT_STOPS]
    || ['#3A2818', '#5C3E28'];
  const gradId = `sg_${(artStyle ?? 'default').replace(/[^a-z0-9]/gi, '_')}`;
        ...
        <svg viewBox={`0 0 100 ${vbH}`} width="100%" style={{ display: 'block', borderRadius: 2 }}>
          <defs>
            <linearGradient id={gradId} x1="0" y1="0" x2="1" y2="1">
              ...
              {gradientStops[2] && <stop offset="50%" stopColor={gradientStops[1]}/>}
              <stop offset="100%" stopColor={gradientStops[2] ?? gradientStops[1]}/>
            </linearGradient>
          </defs>
          {/* Polaroid white frame */}
          <rect x="4" y="4" width="92" height={vbH - 12} rx="2" fill="#F8F4EC" filter="url(#shadow)"/>
          {/* Scene area */}
          <rect x="8" y="8" width="84" height={vbH - 30} rx="1" fill={`url(#${gradId})`}/>
  ...
  return (
    <div style={{ width: maxW, maxWidth: '100%' }}>
      <svg viewBox={`0 0 100 ${vbH}`} width="100%" style={{ display: 'block', borderRadius: 2 }}>
        ...
        {/* Scene background */}
        <rect x="0" y="0" width="100" height={vbH} fill={`url(#${gradId})`}/>
        {bloom}
